...
describe('Date Utils', () => {
  ...
  describe('getDaysBetweenDateRange function', () => {
    it('should return inclusive days', () => {
      const start = new Date('2023-04-01');
      const end = new Date('2023-04-05');

      expect(getDaysBetweenDateRange(start, end, true)).toEqual(5);
    });

    it('should return exclusive days', () => {
      const start = new Date('2023-04-01');
      const end = new Date('2023-04-05');

      expect(getDaysBetweenDateRange(start, end, false)).toEqual(3);
    });
  });
});
